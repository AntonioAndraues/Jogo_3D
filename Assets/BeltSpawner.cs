...
{
    //https://www.youtube.com/watch?v=w422-JEQ8Og
    //referencia deste video

    [Header("Spawner Settings")]
    public static GameObject[] asteroids;
    ...
    private void Start()
    {
        // Adiciona textura novamente
        FindObjectOfType<MapGenerator>().GenerateMap();

        asteroids = Resources.LoadAll<GameObject>("Asteroids_Pack/Assets/Prefabs");
        int len_asteroids = asteroids.Length - 1;
        int witch;
        ...
        for (int i = 0; i < asteroidDensity; i++)
        {
            do
            {
                witch = Random.Range(0, len_asteroids);
            ...
            GameObject _asteroid = Instantiate(asteroids[witch], worldPosition, Quaternion.Euler(Random.Range(0, 360), Random.Range(0, 360), Random.Range(0, 360)));
